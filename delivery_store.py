...
def fingerprint(row, mode):
    ...
    content = {"student": row.student_id or row.student_name, "phone": row.parent_phone, "date": row.date_text,
...
class DeliveryStore:
    ...
    def reserve(self, keys):
        ...
        with self.connection() as db:
            ...
            for key in keys:
                item = db.execute("SELECT state FROM deliveries WHERE fingerprint=?", (key,)).fetchone()
                if item and item[0] in BLOCKING:
                    blocked.append(key)
                    continue
                db.execute("""INSERT INTO deliveries VALUES (?, 'pending', ?, '')
    ...
    def finish(self, keys, state, batch_id=""):
        ...
        with self.connection() as db:
            db.executemany("UPDATE deliveries SET state=?,updated_at=?,batch_id=? WHERE fingerprint=?",
                           [(state, datetime.now(timezone.utc).isoformat(), str(batch_id), key) for key in keys])
